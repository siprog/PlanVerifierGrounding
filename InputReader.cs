                ...
                else if (state == State.ordering)
                {
                    ...
                    if (!line.Trim().Equals(")"))
                    {
                        if (TOWasTrueBeforeThisTask && !firstOrderingLaterCondition)
                        {
                            ...
                            //This is done in such a complicated way because its possible that someone will name tasks but not put any ordering later. 
            ...
            foreach (Tuple<List<int>, Term, bool> cond in betweenconditions)
            {
                for (int i = 0; i < cond.Item2.Variables.Length; i++)
                {
                    ...
                    if (j == -1 || j > methodParams.Count - 1)
                    {
                        ...
                        //If it doesnt start with a ? then it is a constant so of course its not in the main rule. It will be added later on. No need to call warning
                    }
                }
        ...
        public List<Term> ReadProblem(String fileName, Dictionary<String, ConstantType> allConstantTypes, ref Dictionary<String, Constant> constants, out Rule goalRule, out List<Term> goalState)
        {
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                else if (inObjects)
                {
                    ...
                    {
                        inObjects = false;
                        AddNewConstants(inputConstants, ref constants); //Adds inputconstants in constants. Check uniqueness and substitute constants with type any if possible.                         
                    }
